...
#creating a new table with relevant EPC ratings for each property from Dexters
...
##    merged_table
    ...
    for url in merged_table['url'].tolist():
        ...
        if len(epc_rating)!=0:
            epc_avg.append(Counter(epc_rating).most_common()[0][0])
##            try:
##                epc_avg.append(mode(epc_rating))
##            except StatisticsError:
##                epc_avg.append(mode(epc_rating).mode[0])

            
                
##            mode_list=[]
##            mode_list=mode_list.append(mode(epc_rating))
##            #if two equally common modes are found, then choose the maximum value which represents the most efficient one
##            if len(mode_list)>1:
##                epc_avg.append(mode_list[0])
##            else:
##                epc_avg.append(mode(epc_rating))
        else:
            epc_avg.append(None)
...
create_schema >> web_scraping_task_dexters >> save_result_to_postgres_db_dexters >> get_sql_merged_table
